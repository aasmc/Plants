...
class CacheOnSuccess<T : Any>(
    ...
    suspend fun getOrAwait(): T {
        return supervisorScope {
            // this function is thread-safe if deferred is Volatile and all reads
            // and writes hold the mutex
    ...
    private suspend fun Deferred<T>.safeAwait(): T {
        try {
            // note that this call to await will always throw if this coroutine is cancelled
            return await()
        } catch (t: Throwable) {
            // if deferred still points to 'this' instance of Deferred, clear it, since
